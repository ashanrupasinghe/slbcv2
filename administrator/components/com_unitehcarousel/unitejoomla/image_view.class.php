...
defined('_JEXEC') or die;
	...
	class UniteImageViewHCar{
		...
		/**
		 * get thumb filename from the input
		 * 
		 */		
		public static function getThumbFilenameFromInput($filename,$width,$height,$exact){
						
			$info = pathInfo($filename);
			$ext = UniteFunctionsHCar::getVal($info, "extension");
			
			$name = $info["filename"];
			$thumbFilename = $name."_".$width."x".$height;
			
			if($exact == true) 
				$thumbFilename .= "_exact";
		...
		private function validateFilename($filename){
			...
			$info = pathinfo($filename);
			$extension = UniteFunctionsHCar::getVal($info, "extension");
